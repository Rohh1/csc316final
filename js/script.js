...
const sections = [
    ...
    { id: 'dashboard', correct: 'C', init: initDashboardVis, next: 'force-galaxy' },
...
document.querySelectorAll('.mcq-option').forEach(option => {
    option.addEventListener('click', function() {
        ...
        document.querySelectorAll(`.mcq-option[data-quiz="${quizId}"]`).forEach(o => o.classList.remove('selected'));
...
// Attach click listeners to all Continue buttons
document.querySelectorAll('.continue-btn').forEach(btn => {
    // Determine the section ID from the button's parent structure
    const parentVis = btn.closest('.vis-content');
    const parentQuiz = btn.closest('.quiz-block');
    let sectionId = parentVis ? parentVis.id.replace('vis-', '') : (parentQuiz ? parentQuiz.id.replace('quiz-', '') : null);

    if (sectionId && btn.id.startsWith('continue-')) {
        btn.addEventListener('click', function() {
            const section = sections.find(s => s.id === sectionId);
            if (section) {
                revealVis(section.id);
                if (section.id !== 'orbit') {
                    // Reveal next section's quiz
                    const nextSectionElement = document.getElementById(`section-${section.next}`);
...
function checkAnswer(id, selected) {
    ...
    const mcqOptions = document.querySelectorAll(`.mcq-option[data-quiz="${id}"]`);
...
function revealVis(id) {
    document.getElementById(`quiz-${id}`).classList.add('hidden');
    document.getElementById(`vis-${id}`).classList.remove('hidden');
    ...
    // Scroll to the revealed visualization content
    scrollToSection(`#vis-${id}`);
}

// --- VISUALIZATION INITIALIZATION FUNCTIONS ---

// 1. Map Overview (FULL Logic Integrated - from csc316final/js/map.js)
function initMapVis() {
    if (d3.select("#map svg").size() > 0) return;
    ...
    const projection = d3.geoNaturalEarth1()
        .scale(190) // Adjusted scale for new width
    ...
    // NOTE: d3.json points to an external resource for the world map geometry.
    Promise.all([
        ...
        d3.select("#map").append("div")
            ...
            .text("Error loading map or data. Check CSV path and world-atlas link.");
    });
}
...
function initFlowsVis() {
    ...
    function updateBars(regionIndex, targetIndex = null) {
        ...
        data.forEach(d => {
            ...
            if (flowMode === "outbound") {
                gradient.append("stop").attr("offset", "0%").attr("stop-color", color(continents[regionIndex]));
                gradient.append("stop").attr("offset", "100%").attr("stop-color", color(d.target));
            } else {
                gradient.append("stop").attr("offset", "0%").attr("stop-color", color(d.target));
                gradient.append("stop").attr("offset", "100%").attr("stop-color", color(continents[regionIndex]));
            }
        });
        ...
        labels.join(
            enter => enter.append("text")
                ...
                .text(d => `${d.target} (${d.value}k)`)
                .style("font-size", "13px").style("fill", "#ccc").style("font-weight", "500"),
            update => update
                .attr("x", d => x(d.value) + labelPadding)
                .attr("y", d => y(d.target) + y.bandwidth()/2)
                .text(d => `${d.target} (${d.value}k)`),
    ...
    function showDefaultBars() {
        const matrix = getMatrix(flowMode);
        const totals = matrix.map((row, i) => ({ index: i, total: row.reduce((sum, val) => sum + val, 0) }));
...
function initDashboardVis() {
    ...
    d3.csv("data/cia_factbook.csv", d3.autoType).then(raw => {
        ...
        function updateStyles() {
            ...
            dotsScatter
                .attr("stroke-width", d => brushedCountries.has(d.country) ? 2.5 : 1.3)
                .attr("stroke", d => brushedCountries.has(d.country) ? "#fff" : color(d.healthTier))
                .attr("opacity", d => {
...
function initOrbitVis() {
    ...
    const regionColors = {
        ...
        'North America': '#9b59b6', 'South America': '#1abc9c', 'Oceania': '#16a085',
        'Middle East': '#e67e22', 'Caribbean': '#2ecc71', 'Central Asia': '#95a5a6',
        'Unknown': '#bdc3c7'
    };

    const countryToRegion = {
        'Albania': 'Europe', 'China': 'Asia', 'India': 'Asia', 'Nigeria': 'Africa', 
        'United States': 'North America', 'Brazil': 'South America', 'Saudi Arabia': 'Middle East',
        'Kazakhstan': 'Central Asia', 'Australia': 'Oceania', 'Cuba': 'Caribbean',
        'Austria': 'Europe', 'Belgium': 'Europe', 'Bulgaria': 'Europe', 'Croatia': 'Europe', 'Czech Republic': 'Europe', 'Denmark': 'Europe', 'Estonia': 'Europe', 'Finland': 'Europe', 'France': 'Europe', 'Germany': 'Europe', 'Greece': 'Europe', 'Hungary': 'Europe', 'Iceland': 'Europe', 'Ireland': 'Europe', 'Italy': 'Europe', 'Latvia': 'Europe', 'Lithuania': 'Europe', 'Luxembourg': 'Europe', 'Netherlands': 'Europe', 'Norway': 'Europe', 'Poland': 'Europe', 'Portugal': 'Europe', 'Romania': 'Europe', 'Slovakia': 'Europe', 'Slovenia': 'Europe', 'Spain': 'Europe', 'Sweden': 'Europe', 'Switzerland': 'Europe', 'United Kingdom': 'Europe', 'Ukraine': 'Europe', 'Belarus': 'Europe', 'Moldova': 'Europe', 'Serbia': 'Europe', 'Bosnia and Herzegovina': 'Europe', 'Montenegro': 'Europe', 'North Macedonia': 'Europe', 'Kosovo': 'Europe', 'Japan': 'Asia', 'South Korea': 'Asia', 'North Korea': 'Asia', 'Indonesia': 'Asia', 'Thailand': 'Asia', 'Vietnam': 'Asia', 'Philippines': 'Asia', 'Myanmar': 'Asia', 'Malaysia': 'Asia', 'Singapore': 'Asia', 'Bangladesh': 'Asia', 'Pakistan': 'Asia', 'Sri Lanka': 'Asia', 'Nepal': 'Asia', 'Cambodia': 'Asia', 'Laos': 'Asia', 'Mongolia': 'Asia', 'Bhutan': 'Asia', 'Brunei': 'Asia', 'Timor-Leste': 'Asia', 'Taiwan': 'Asia', 'Hong Kong': 'Asia', 'Macao': 'Asia', 'Ethiopia': 'Africa', 'Egypt': 'Africa', 'South Africa': 'Africa', 'Kenya': 'Africa', 'Tanzania': 'Africa', 'Uganda': 'Africa', 'Algeria': 'Africa', 'Sudan': 'Africa', 'Morocco': 'Africa', 'Angola': 'Africa', 'Ghana': 'Africa', 'Mozambique': 'Africa', 'Madagascar': 'Africa', 'Cameroon': 'Africa', 'Niger': 'Africa', 'Mali': 'Africa', 'Burkina Faso': 'Africa', 'Malawi': 'Africa', 'Zambia': 'Africa', 'Senegal': 'Africa', 'Somalia': 'Africa', 'Chad': 'Africa', 'Zimbabwe': 'Africa', 'Guinea': 'Africa', 'Rwanda': 'Africa', 'Benin': 'Africa', 'Tunisia': 'Africa', 'Burundi': 'Africa', 'Togo': 'Africa', 'Sierra Leone': 'Africa', 'Libya': 'Africa', 'Liberia': 'Africa', 'Mauritania': 'Africa', 'Central African Republic': 'Africa', 'Eritrea': 'Africa', 'Namibia': 'Africa', 'Botswana': 'Africa', 'Lesotho': 'Africa', 'Congo': 'Africa', 'Gabon': 'Africa', 'Guinea-Bissau': 'Africa', 'Equatorial Guinea': 'Africa', 'Mauritius': 'Africa', 'Eswatini': 'Africa', 'Djibouti': 'Africa', 'Comoros': 'Africa', 'Cape Verde': 'Africa', 'Sao Tome and Principe': 'Africa', 'Seychelles': 'Africa', 'Canada': 'North America', 'Mexico': 'North America', 'Argentina': 'South America', 'Colombia': 'South America', 'Peru': 'South America', 'Venezuela': 'South America', 'Chile': 'South America', 'Ecuador': 'South America', 'Bolivia': 'South America', 'Paraguay': 'South America', 'Uruguay': 'South America', 'Guyana': 'South America', 'Suriname': 'South America', 'Iran': 'Middle East', 'Iraq': 'Middle East', 'Turkey': 'Middle East', 'Yemen': 'Middle East', 'Syria': 'Middle East', 'Jordan': 'Middle East', 'United Arab Emirates': 'Middle East', 'Israel': 'Middle East', 'Lebanon': 'Middle East', 'Oman': 'Middle East', 'Kuwait': 'Middle East', 'Qatar': 'Middle East', 'Bahrain': 'Middle East', 'Palestine': 'Middle East', 'Afghanistan': 'Middle East', 'Uzbekistan': 'Central Asia', 'Turkmenistan': 'Central Asia', 'Kyrgyzstan': 'Central Asia', 'Tajikistan': 'Central Asia', 'Azerbaijan': 'Central Asia', 'Armenia': 'Central Asia', 'Georgia': 'Central Asia', 'Russia': 'Central Asia', 'New Zealand': 'Oceania', 'Papua New Guinea': 'Oceania', 'Fiji': 'Oceania', 'Solomon Islands': 'Oceania', 'Vanuatu': 'Oceania', 'Samoa': 'Oceania', 'Kiribati': 'Oceania', 'Tonga': 'Oceania', 'Micronesia': 'Oceania', 'Palau': 'Oceania', 'Marshall Islands': 'Oceania', 'Nauru': 'Oceania', 'Tuvalu': 'Oceania', 'Haiti': 'Caribbean', 'Dominican Republic': 'Caribbean', 'Jamaica': 'Caribbean', 'Trinidad and Tobago': 'Caribbean', 'Bahamas': 'Caribbean', 'Barbados': 'Caribbean', 'Saint Lucia': 'Caribbean', 'Grenada': 'Caribbean', 'Saint Vincent and the Grenadines': 'Caribbean', 'Antigua and Barbuda': 'Caribbean', 'Dominica': 'Caribbean', 'Saint Kitts and Nevis': 'Caribbean',
    };
    ...
    d3.csv('data/democracy_data_clean.csv').then(data => {
        ...
        data = data.filter(d => !isNaN(d.spatialElectoral) && !isNaN(d.spatialDemocracy));
        ...
        Object.keys(regionColors).forEach(region => {
            ...
            item.append('div').attr('class', 'legend-color').style('background-color', regionColors[region]);
            item.append('span').text(region);

            item.style('cursor', 'pointer')
                .on('click', () => {
                    window.expandedRegion = window.expandedRegion === region ? null : region;
        ...
        function calculateClusters(yearData) {
            const clusters = d3.rollup(
                yearData,
                countries => ({
                    countries: countries, count: countries.length,
        ...
        function updateVisualization(year) {
            ...
            if (viewMode === 'cluster' && !window.expandedRegion) {
                ...
                const clusterElements = clustersGroup.selectAll('.cluster')
                    ...
                    .join(
                        enter => enter.append('circle').attr('class', 'cluster').attr('r', 0)
                            .attr('cx', d => xScale(d.avgElectoral)).attr('cy', d => yScale(d.avgDemocracy)),
                        update => update,
                ...
                if (window.expandedRegion) {
                    displayData = yearData.filter(d => d.region === window.expandedRegion);
                }

                planetsGroup.selectAll('.planet')
                    .data(displayData, d => d.country)
                    .join(
                        enter => enter.append('circle').attr('class', 'planet').attr('r', 0)
                            .attr('cx', d => xScale(d.spatialElectoral)).attr('cy', d => yScale(d.spatialDemocracy)),
                        update => update,
                        exit => exit.transition().duration(300).attr('r', 0).remove()
                    )
                    .transition().duration(300)
